...
page_fault(struct intr_frame *f)
{
   ...
   if ((fault_addr >= PHYS_BASE || fault_addr < (void *)(0x08048000)) || !not_present)
   {
      exit(-1);
   }

   void *VA_for_faulted_page = (void *)((uint32_t)fault_addr & 0xfffff000);
   struct sup_page_table_entry *tmp = malloc(sizeof(*tmp));
   tmp->spte_VA_for_page = VA_for_faulted_page;
   struct hash_elem *in_sup_page_table = hash_find(&thread_current()->sup_page_table, &tmp->elem);

   if (in_sup_page_table == NULL)
   {
      if (user)
      {
         esp = f->esp;
      ...
      if (!decide_stack_growth_and_do_if_needed(esp, fault_addr))
      {
         exit(-1);
      }
   }
   ...
   {
      struct sup_page_table_entry *fault_page_spte = hash_entry(in_sup_page_table, struct sup_page_table_entry, elem);
      ...
      if (fault_page_spte->current_page_location != InMemory)
      {
         ...
         ft_entry->fte_VA_for_page = VA_for_faulted_page;
         ...
         pagedir_set_page(ft_entry->fte_thread->pagedir, ft_entry->fte_VA_for_page, ft_entry->fte_kernel_VA_for_frame, fault_page_spte->writable);
